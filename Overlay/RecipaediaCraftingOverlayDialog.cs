...
namespace RecipaediaEX.Overlay {
    ...
    public class RecipaediaCraftingOverlayDialog : Dialog, IRecipaediaRecipeNavigator {
        ...
        void ShowPreviewForItem(IRecipaediaRecipeItem recipeItem, int? tabIndex = null) {
            ...
            if (selectedIndex >= 0) m_recipePreview.DisplayRecipes(m_crafterGroups[selectedIndex].Recipes);

            else m_recipePreview.DisplayRecipes([]);
        ...
        void OnCrafterTabSelected(int index) {
            ...
            if (index < 0 || index >= m_crafterGroups.Count) return;
        ...
        void PushCurrentPreviewState() {
            ...
            if (m_lastPreviewItem is not IRecipaediaRecipeItem current || !m_recipeDetailPopup.IsVisible) return;
