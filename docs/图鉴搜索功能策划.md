...
**环境**：工业时代 2 + RecipaediaEX 已部署；进入图鉴主屏 `RecipaediaEXScreen`；默认在 **All Blocks**（或条目最多的全量分类）。每条用例 **单独执行**（先 `×` 清空或切换分类复位，再输入并点搜索图标）。
...
**验收记录**：2026-06-08 手工冒烟 **A–F 通过**（工业时代 2 + REX 当前 main）。
