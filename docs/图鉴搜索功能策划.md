...
| 条目类型 | 不限方块：`IRecipaediaItem`（方块 / 流体 / 自定义实体等） |
...
依赖 REX 的内容模组通常会显著扩充方块 / 流体 / 自定义图鉴条目、配方类型与工作站点。玩家在「找某个产物」「某原料能做什么」「某机器相关物品」时，现有 UI 成本过高。
...
| # | 议题 | 决策 |
|---|------|------|
| 1 | **搜索范围** | **仅当前分类**。用户处于「All Blocks」等全量分类时等价于全局；切换到「Fluid」等子分类时，搜索只在 Fluid 条目内过滤。切换分类时保留或清空搜索词见 §5.2（默认：**清空搜索**）。 |
...
| `Fluid`、`Terrain`、自定义分类 | 仅该分类 `GetItems()` |
| 切换分类 | **默认清空搜索框与 Filter 状态**，重新 `PopulateBlocksList`；避免「Fluid 分类仍显示上一次的方块搜索结果」 |
...
| US-02 | 切换到 Fluid 后搜索「水」 | 仅流体条目；不含方块 |
| US-03 | Fluid 分类下搜「铜」 | 无结果或仅 Fluid 内匹配项，不出现方块 |
...
- Picker 选的物品解析为 blockValue / fluidValue，再查索引倒排表。
...
- [ ] `RecipaediaSearchIndex` + `RecipesReset` / Categories 挂钩
- [ ] `RecipaediaSearchParser` + `RecipaediaSearchEngine`
- [ ] `RecipaediaEXScreen` 集成社区同款搜索行（`key` / `Search` / `SearchType` / `ClearSearchLink`）
- [ ] `RecipaediaSearch` 语言键（placeholder、筛选摘要）
- [ ] `RecipaediaSearchFilterDialog` + XML
- [ ] Token：`@t:` `@pack:` `@mod:` `@crafter:` `@in:` `@out:` `@uses:` `@makes:` `@rtype:` `@has:recipe` `@can-use` `-` `name=` / `name:`
- [ ] 仅当前分类候选集过滤
- [ ] 切换分类清空搜索
- [ ] 修复条目配方计数 stale
...
| 搜索概念 | REX API |
|----------|---------|
| 产物侧条目 | `IRecipaediaRecipeItem.Match` + `RecipeExtraKeys.MatchedResultBlockValues` / `MatchedResultFluidValues` |
