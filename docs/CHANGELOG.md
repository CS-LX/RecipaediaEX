...
**对比基准：** `2.0.0.0`（Git 标签 `v2.0.0.0`）→ `2.0.0.1`（Git 标签 `v2.0.0.1`）

### 修复

- **进档后方块 Index 重分配后，图鉴 / 合成助手配方预览错挂其他模组物品**：`RecipesReset` 时清空 `RecipaediaCategoryCatalog`、Overlay 原料展开缓存与缺料文案缓存，避免继续使用菜单期冻结的绝对 `BlockValue`。
- **`BlockItem.Match` / `IsIngredient`**：改为按方块**类型 + data**识别条目，不再单独信任绝对 int 相等（重分配后数字可能撞车，导致「能合成但预览产物图标错误」）。

### 适配指南（从 `2.0.0.0` 升级）

1. **依赖版本** — 内容模组 `modinfo` 中 `com.recipaediaex` 可写 **`2.0.0.1`**，或继续写 **`2.0.0.0` / `>=2.0.0.0`**（宿主按 NuGet 范围满足即可）。
2. **无需改代码** — 本版为 REX 内部缓存与 Match 修正；依赖方一般只需替换 `.scmod`。
3. 若内容模组自行缓存了 `IRecipaediaItem` / 绝对 `BlockValue`，仍须在进档 / 配方重载后自行失效。
