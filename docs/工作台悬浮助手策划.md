...
> **版本**：v0.21  
> **状态**：**Pre-release 发版前冲刺** — **Track C 冒烟已通过**（2026-06-19）；待发 **B4**（modinfo / CHANGELOG / tag）  
> **下一里程碑**：B4 对外 prerelease 发包 + 群公告  
...
**验收记录**：生存模式手工冒烟；包体 `SCIE.scmod` + `RecipaediaEX.scmod`（`preview5`）成对加载。
...
- [ ] **B4** modinfo / CHANGELOG / tag；双 mod 发包
