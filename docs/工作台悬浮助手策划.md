...
| **IE2 侧** | 主仓 `modinfo` 依赖 **`2.0.0.0`**、子模块指针、SCIE 发包 — **不由 REX 仓维护**；Host 接入进度见主仓 `docs/guides/合成助手-工业机器接入清单.md`（2026-07-13：**15** 台已接，13 台合成机搁置） |
...
| **下一优先（IE2）** | 按接入清单分批接剩余合成机 Host；**W6/4c 仍搁置**（REX 2.0.x 方案评审后再做） |
...
| 自动摆放 | Phase 2a/2b + W2/W3 已交付 | ✅ 有形合成、熔炼输入区、One2One / 反应釜 / 绞线机双槽等（IE2 **15** Host，见接入清单） |
...
| 工业机器（反应釜 / One2One / 绞线机等） | Phase 2b + IE2 扩展 | ✅ 预览 + **`+`**（**15** Host，见接入清单） |
...
| IE 铁炉 / 电炉 / 燃气炉等 | W3 | 同上 `FurnacePlacementTarget`（SCIENEW Widget 接 Host） |
...
| One2One 单槽 | `ContainerSlot` | 0 | 内容模组 Target → **个数**（压板机等，**≠ 熔炉**） |
...
| Phase 2b | `One2OneRecipe` + 压板机等 | 单输入槽（One2One，与熔炉无关） |
| Phase 2c | `OriginalSmeltingRecipe` + `ComponentEXFurnace` / `ComponentIEFurnace` | **熔炼输入区** `+`：多格、多料、**Transform 平移**；仅写输入格（不写燃料/产物/残渣） |
...
**动机**：preview8 已交付工作台/机床/反应釜/One2One 的 `+`，但 **熔炉面板无 Host**、`OriginalSmeltingRecipe` 无 `IPlacableRecipe`，玩家开炉仍无法同屏助手 + 摆料。社区与黑盒共识：**熔炉不是「单槽 One2One」**——IE 铁炉/电炉常见 **2～4 个输入格**，炼钢/合金等配方为 **多料 + 格位**（如 `"aab"`、`"abb"`），且匹配管线 **支持 Transform 平移**（玩家可手动在输入格间平移图案）。
...
- [x] IE2：铁炉 / 电炉 / 燃气炉 Widget 接 `IRecipaediaOverlayHost` + `FurnacePlacementTarget`
- [x] 主仓 `docs/guides/合成助手-工业机器接入清单.md` 增补熔炉行
...
**验收记录**（2026-07-12）：生存模式手工黑盒 **US-C2c-01～05 全通过**；原版熔炉 + IE2 铁炉/电炉/燃气炉；Track C 工作台/One2One 回归无回归。
...
| v0.28 | 2026-07-11 | **W2 交付**：长按 `+` 连续放置（`PlacementLongPressRepeater`）；有形合成格广度优先；`PlaceRecipe(clearGridBeforePlace, showFeedback)`；i18n `:14` 悬停提示；One2One 灌槽见 IE2 `ContainerSlotPlacementPlanner` |
...
| v0.30 | 2026-07-11 | **W3 交付**：`FormattedGridPlacementPlanner`、`FurnacePlacementTarget`、`RecipaediaFurnaceWidget`；IE2 铁炉/电炉/燃气炉 Host；CHANGELOG Unreleased |
