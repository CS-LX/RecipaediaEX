...
RecipaediaEX/
  Overlay/
    ...
    IPlacableRecipe.cs
    PlacementRequirement.cs          # ItemSlot / FluidTank / ShapeCell …
...
public readonly struct PlacementRequirement {
    public PlacementKind Kind;           // ItemSlot, FluidTank, ShapeCell
    public int TargetSlotIndex;
    public string MatchKey;              // CraftingId / FluidId
    public int ItemCount;
    public float FluidVolume;
    public int[] AcceptedBlockValues;    // 等价物
}

public enum PlacementKind { ItemSlot, FluidTank, ShapeCell }
```

| 配方类型 | `IPlacableRecipe` 实现 | 说明 |
|----------|-------------------------|------|
| `OriginalCraftingRecipe` | REX 内置适配 | 36 格 + Transform |
| `ReactionEquationRecipe` | SCIENEW | 物品槽 0–2 + 流体罐 0–2 |
| `One2OneRecipe` | SCIENEW | 单输入槽 |
...
public readonly struct PlacementSources {
    ...
    public ITank? ContainerTanks;          // 反应釜等
}
```

**REX 默认实现 `CraftingTablePlacementTarget`**（`ComponentEXCraftingTable`）：

1. `recipe` 为 `IPlacableRecipe` → 读取 `ShapeCell` 需求或回退 36 格 `Ingredients`。
...
**内容模组 `ReactorPlacementTarget`**（`ComponentReactor`）：

1. 解析 `ReactionEquationRecipe` → `ItemSlot` + `FluidTank` 需求。
2. 物品：填 0–2 输入槽；等价物选择（基元 P3）。
3. 流体：罐体可写则 `AddTankFluid`；否则记入 `Missing`（不 silent fail）。
4. `execute=true` 后调用 `FindEquation()`。

**禁止**：在 REX 核心写反应釜/灌装机等工业逻辑 → 内容模组实现 Target 并在 Widget 的 `GetPlacementTarget()` 返回。
...
原 v0.1「Phase 3 化工不做 +」**已废止**；化工/流体通过 `IPlacableRecipe` + 内容模组 Target 接入，REX 只提供协议。
...
6. **联机**（P8）：与手工拖放同一套 `IInventory`/`ITank` API。
