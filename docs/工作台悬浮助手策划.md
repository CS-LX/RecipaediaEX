...
> **版本**：v0.12  
> **状态**：**Phase 1 已收尾**（2026-06-16）；**Phase 1.5 / 1.6 已收尾**；**Phase 2a 已收尾**；**Phase 2a.1 已收尾**；**Phase 2a.2 已收尾**；**Phase 2b 已收尾**（2026-06-16，工业摆放 + 验收期 bugfix + 性能优化）  
...
|------|------|
| 合成助手开/关 | 不持久化（会话态） |
| 助手内 **分类 + 列表滚动** | **会话态**（`RecipaediaCraftingOverlaySessionState`）；toggle 关闭再开恢复；**不**写入世界存档 |
...
| **D23 记忆** | `RecipaediaCraftingOverlaySessionState`：toggle 关助手时写入 **分类 id + 列表滚动**；再开恢复 |
...
**明确不做（本阶段）**：条带布局改建；搜索性能优化（独立任务）

---

### Phase 2a.3 — 会话扩展（P2，可选）

- [ ] Toggle **Hide 不 Destroy**（减少重建开销；记忆已由 D23 覆盖主诉求）
- [ ] 记住搜索词（若用过）
...
#### 技术债 TD-01：`IRecipaediaNamedItem`（过渡，日后废除）

**根因**：合成助手详情标题 `GetRecipeItemTitle` 优先读 `IRecipaediaDescriptionItem.Name`（与全屏 `RecipaediaEXDescriptionScreen` 一致）。`BlockItem`、`BestiaryItem` 等已接入；**`FluidItem` 遗漏**，仅有独立详情屏 `FluidRecipaediaDescriptionScreen`，未实现 `IRecipaediaDescriptionItem`，回退到 `ToString()` 即类名。

**临时修复（Phase 2b）**：

- REX 新增 **`IRecipaediaNamedItem`**（仅 `Name`），`FluidItem` 实现该接口；
- `GetRecipeItemTitle` 在 `IRecipaediaDescriptionItem` 之后回退读 `IRecipaediaNamedItem`。

**正式方案（后续清理，建议 Phase 2c 或图鉴条目统一重构时做）**：

- [ ] **`FluidItem` 补齐 `IRecipaediaDescriptionItem`**：`Name`、`Icon`（`FluidWidget`）、`Description`、`GetProperties`（可与 `FluidRecipaediaDescriptionScreen` 共用属性逻辑）；
- [ ] **删除 `IRecipaediaNamedItem`** 及 `GetRecipeItemTitle` 中对应分支；
- [ ] 约定：凡在合成助手 / 全屏图鉴中作为「可命名条目」展示的 `IRecipaediaItem`，**统一经 `IRecipaediaDescriptionItem` 提供显示名**，不再新增仅含 `Name` 的平行接口。
