...
> **版本**：v0.29  
> **状态**：**preview8 已发** — REX `2.0.0.0-preview8`（2026-06-25）；**W2 长按连放已交付**（待 preview9 发包）  
> **下一里程碑**：**W3**（熔炼输入区 Phase 2c）；**W6 搁置**（见下）  
...
| **代码锚点** | Overlay：`RecipaediaCraftingOverlayDialog` / `Controller`；Placement：`IRecipePlacementTarget` / `PlacableRecipeAdapter` / `CraftingGridPlacementPlanner`（W3 升格为 `FormattedGridPlacementPlanner`）；搜索：`RecipaediaSearchEngine` |
...
| **W3** | **Phase 2c 熔炼输入区**（`FormattedGridPlacementPlanner` + 熔炉 Host） | 按热度 | 1～2d |
...
- [ ] REX：`FormattedGridPlacementPlanner`（自 `CraftingGridPlacementPlanner` 提取；工作台 **零行为回归**）
- [ ] REX：`FormattedGridPlacableRecipe`；`PlacableRecipeAdapter` 注册 `OriginalSmeltingRecipe`
- [ ] REX：`FurnacePlacementTarget`（`ComponentFurnace` / `ComponentEXFurnace`）
- [ ] REX：`RecipaediaFurnaceWidget` + `RecipaediaEXLoader.OnModalPanelWidgetSet` 替换原版 `FurnaceWidget`
- [ ] IE2：铁炉 / 电炉 / 燃气炉 Widget 接 `IRecipaediaOverlayHost` + `FurnacePlacementTarget`
- [ ] 主仓 `docs/guides/合成助手-工业机器接入清单.md` 增补熔炉行
