...
> **版本**：v0.5  
> **状态**：**Phase 1 已收尾**（2026-06-16）；**Phase 1.5 已定稿**（2026-06-16 评审，见 §9.1）；Phase 2a+ 待实现  
...
| D18 | Phase 2a `+` 与 Tab | **暂定**：仅当当前选中 Tab 的 Crafter 与 Host 一致时启用 `+`；实施 Phase 2a 时视 playtest **可再调整** |
...
| US-C03 | 在 4×4 机床 / 5×5 车间打开助手 | 搜索仍全局；选中条目后预览 **含** 可放入当前网格的较小配方（`recipeWidth <= GridWidth`） |
...
> **注**：Phase 1 中 `@crafter` + `GridWidth` 硬过滤、预览仅当前工作站等设计 **偏死板**；**Phase 1.5（§9.1）已定稿**，实现完成后回写 §3–§6 与基元 V4。

---

### Phase 1.5 — 预览松绑与 Crafter 分页（已定稿）
...
│ [×]  铁块                                              │
│ ┌ Tab ─────────────────────────────────────────────┐  │
│ │ [工作台icon] [机床icon] [熔炉icon] [反应釜icon] … │  │  ← 仅展示「该条目在此 Crafter 下 ≥1 条配方」的 Tab
│ └──────────────────────────────────────────────────┘  │
│ ┌ Descriptor 纵向 Scroll ──────────────────────────┐  │
│ │  ┌──────────────┐                                 │  │
│ │  │ CraftingRecipe│  （矩阵缩放 0.85）              │  │
│ │  │ Descriptor   │                                 │  │
│ │  └──────────────┘                                 │  │
│ │  … 同 Tab 下多条配方 …                            │  │
│ └──────────────────────────────────────────────────┘  │
...
// 2. 分组：key = Terrain.ExtractContents(crafterBlockValue)
IReadOnlyDictionary<int, List<IRecipe>> byCrafter = GroupByCrafter(all);

// 3. UI：Tab 切换 → m_recipePreview.ShowRecipes(byCrafter[selectedCrafter]);
```

`GroupByCrafter`：对每条配方，遍历 `RecipesCrafterManager.Crafters[recipe]`（或 `ICrafter` 扫描），按 **方块 Index** 去重归入多 Tab（同一配方可出现在工作台 Tab 与机床 Tab，若两者 `IsCrafter` 均为真）。
...
- [ ] `RecipaediaCraftingContext`：删除 `GridWidth`；文档与 Host 签名同步  
- [ ] `RecipaediaOverlayRecipeResolver`：`ResolveAllRecipes` + `GroupRecipesByCrafter`；删除 `GetCraftingGridWidth`  
- [ ] `RecipaediaOverlayCrafterTabBar` + 二级弹窗 XML 布局  
- [ ] `RecipaediaCraftingOverlayDialog`：条目点击 → 建 Tab → 默认 Tab → `ShowRecipes`  
- [ ] SCIENEW `CraftingOverlayHostHelper` / 各 Host：去掉 `gridWidth` 实参  
- [ ] 内联 `RecipaediaCraftingTableContextHelper` → 删除文件  
- [ ] 回写 §3 设计目标 V4、§4.2、§6.3–6.4、§8 US-C03；**不修改**全屏图鉴与 [`合成助手-JEI对标与基元语句.md`](合成助手-JEI对标与基元语句.md)（Overlay 实现完成后再单开文档修订，若需要）  
