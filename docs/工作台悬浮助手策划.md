...
> **下一里程碑**：反馈期 feature（§9.PR.5）— **W1 Hide 不 Destroy** 优先；**W4～W7** 纳入社区「酸菜小咸鱼」反馈（§1.2、§9 Phase 4）  
...
| **下一优先** | §9.PR.5 **W1** Hide 不 Destroy → **W4** Overlay 生命周期 → **W5** 全局搜索 → **W6** 原料反查 / 上下文推荐 → **W2** Shift 多组 `+` → **W3** Phase 2c 熔炉 |
...
| 能力 | 现状 | 与玩家诉求 / JEI 的差距 |
|------|------|-------------------------|
| 文本搜索 | `RecipaediaSearchEngine` + 索引 + 高级筛选 | ✅ 已有；❌ 助手 **Phase 1.6 起为分类内搜索**（F6）；❌ 无材料上下文推荐（F7～F9） |
...
| 全屏：**当前图鉴分类** 内搜索 | Overlay：**Phase 1.6 与全屏一致**（分类内）；**Phase 4b 拟修订**为默认 **全局搜 + 可选分类浏览**（D27，F6） |
...
| D27 | Overlay 搜索候选集（F6） | **反馈期修订 Phase 1.6**：助手内 **默认全局搜索**（All Blocks 或等价全条目集）；**Category 条** 改为 **浏览/过滤模式**（翻分类仍限制列表，但 **提交搜索不再被当前分类裁剪**）。全屏图鉴 **仍保持** 分类内搜索（D17 隔离） |
...
| D29 | 打开助手预填列表（F8/F9） | 首次打开（无会话搜索词时）：右侧列表默认 **「可合成推荐」**——由 Host 上报 **槽位 + 绑定背包** 的物品 value 集，经 `@in` 语义或专用索引 **过滤可产出条目**；无材料时回退 **Category 浏览**（与现网 Phase 1.6 一致） |
...
| US-C4-02 | 打开助手且无会话搜索词 | 右侧列表为 **可合成推荐**（基于槽位+背包）；无材料时回退 Category 浏览（D29） |
...
反馈期以 **P0/P1 hotfix** 为主；体验与工程可并行。**2026-06-22** 起纳入社区 **酸菜小咸鱼** 反馈（§1.2、Phase 4）。

| 窗口 | 建议 | 对应反馈 |
|------|------|----------|
| **W1** | hotfix + **Hide 不 Destroy**（2a.3 余项） | — |
| **W4** | **Phase 4a** Overlay 生命周期 + Host 门控回归 | F10、F11（D28、D31） |
| **W5** | **Phase 4b** 助手内 **全局搜索**（修订 Phase 1.6） | F6（D27） |
| **W6** | **Phase 4c** 打开即推荐 + 原料反查入口 | F7～F9（D29、D30） |
| **W2** | **Shift 多组 `+`** 或 **RecipeSlotWidget 联动**（二选一开 feature） | 与 D30 可合并 |
| **W3** | **Phase 2c 熔炉** 或未接 Host 机器（按玩家反馈热度） | — |

**推荐实施顺序**：W1 → **W4**（边界 bug 类，优先）→ **W5**（搜索差评最高频）→ W6 → W2/W3。
...
### Phase 4 — 社区反馈收口（preview6+ · 酸菜小咸鱼 F6～F11）

**动机**：preview6 已交付同屏助手 + `+`，但 **分类内搜索**（Phase 1.6）与 **无材料驱动列表** 使体验仍像「缩小版全屏图鉴」；**生命周期 / Host 边界** 需与 JEI「助手附属合成 GUI」对齐。

**分期**（与 §9.PR.5 W4～W6 对应）：
...
| **空材料** | 回退 Phase 1.6 Category 默认分类浏览 |
