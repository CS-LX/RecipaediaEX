...
> **P0 阻塞**（发 prerelease **前必解**，见 §9.PR.0）：仅 **P0-PR-01** 合成助手搜索卡顿（~~P0-PR-02~~ **已关闭：v0.16 误报**，Phase 2b 后冒烟无 `+` 延迟）  
> **下一里程碑**：P0-PR-01 → Track B 工程整理 + D24/D25 → prerelease 发包  
...
**发包门槛**：**仅 P0-PR-01** 未清偿前不得发 prerelease（见 §9.PR.0）。
...
| ID | 现象 | 影响 | 状态 |
|----|------|------|------|
| **P0-PR-01** | 合成助手内 **搜索提交后主线程卡顿** | US-C02、US-C-PR-01、§10 | **未解决** |
...
| 项 | 说明 |
|----|------|
| **复现（2026-06-16 冒烟）** | 合成助手 → 搜 `铁`：**All Blocks 分类 ≈3s** 卡住；**Items 分类 ≈0.5s**；体感主线程阻塞 |
| **相关代码** | `RecipaediaCraftingOverlayDialog.PopulateBlocksList` → `RecipaediaSearchEngine.Filter(items, categoryId, query)`；全量候选 × `RecipaediaSearchIndex.GetDocument` + 匹配 + 排序；大分类（All Blocks）候选集最大 |
| **与全屏** | 同 `RecipaediaSearchEngine`；Overlay 打开/搜后亦 **全量重建** `ListPanelWidget` 条目，成本叠加 |
| **修复方向（可组合）** | ① Category + query **结果缓存**（同屏重复搜零成本）；② 文本 query **DisplayName 快速剪枝** 再进 Parser；③ `GetDocument` **按 (item, categoryId) 缓存**；④ 匹配结果 **分帧** `AddItem`；⑤ All Blocks 默认策略：打开助手 **默认非 All**（已有 1.6）+ 搜前提示大分类较慢（兜底） |
| **验收** | US-C-PR-01：`铁` @ All Blocks **< 500ms**；`铁` @ Items **< 200ms**；较基线 3s/0.5s **可感知改善** |
...
```text
① P0-PR-01  搜索性能（≈2～3h）          ← 发版门槛，优先
② Track B   工程整理 B1→B3→B2（≈1～1.5h）← 可与 ① 后半并行；发前必做
③ Track A   D24 搜索词 + D25 Tooltip（≈1h）
④ Track C   Rebuild + 冒烟（含搜索基线复测）
⑤ B4 + 发包  modinfo / CHANGELOG / tag / 双 mod
```

**铁律**：**P0-PR-01 过验收** + **Track B 勾选完成** 后发 prerelease；发包后除 P0 回退外不叠新 feature。
...
| 2 | **B3** | **REX / IE2 边界注释** | `IndustrialModLoader`、`PlacableRecipeAdapter`（REX）、`CraftingOverlayIngredientBridge` 注册处各一行职责说明 | 15min |
...
| 7 | **P0-PR-01**：`铁` @ All Blocks **<500ms**、@ Items **<200ms**（较 3s/0.5s 基线） |
| 8 | 工作台 `+` 回归（US-C-PR-02）：有料/缺料无卡顿 |
...
- [ ] **Track B** B1→B3→B2→B5→B4（§9.PR.3）
- [ ] **Track A** Phase 2a.3 子集：D24 + D25（§2a.3.1）
- [ ] Track C 冒烟通过（§9.PR.4）
...
- [ ] **P0-PR-01** Overlay 搜索 / `PopulateBlocksList` 性能（§9.PR.0，**最先**）
- [ ] `RecipaediaCraftingOverlaySessionState`：增加 `SearchQuery`（B2 + D24）
- [ ] `CaptureSessionState` / Dialog 构造：恢复搜索词；换分类仍清空
- [ ] `RecipaediaOverlayDescriptorActionBar`：禁用 `+` 时 Tooltip（D25）
...
| 查询（全屏） | < 100ms / 5000 候选 |
| 查询（**合成助手 Overlay**） | **P0-PR-01**：`铁` @ All Blocks **< 500ms**、@ Items **< 200ms**（基线 2026-06-16：≈3s / ≈0.5s） |
...
| Overlay 搜索卡顿 | **P0-PR-01**：Filter 全量扫描 + List 重建；All Blocks 候选集最大；见 §9.PR.0 |
