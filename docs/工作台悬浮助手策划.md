...
**设计约束（v0.3）**：REX 核心 **不得** 出现 `FluidVolume`、`FluidTank`、`ITank` 等流体/内容模组专有字段或枚举值——与 [`recipaedia-rex-anti-patterns.mdc`](../../../.cursor/rules/recipaedia-rex-anti-patterns.mdc) §1 一致。流体、罐体、化工方程的 **Quantity 语义与槽位物理含义** 由 **内容模组** 的 `IPlacableRecipe` 适配器 + `IRecipePlacementTarget` 实现解释；REX 只提供 **与领域无关的寻址 + 数量** 载体。
