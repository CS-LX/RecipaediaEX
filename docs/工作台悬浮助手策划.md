...
> **版本**：v0.16  
> **状态**：**Phase 1～2b 已收尾**；**TD-01 已清偿**；**Pre-release 发版前冲刺**（§9，**进行中**）  
> **P0 阻塞**（发 prerelease **前必解**，见 §9.PR.0）：**P0-PR-01** 合成助手搜索卡顿；**P0-PR-02** 工作台 `+` 点击延迟  
> **下一里程碑**：清偿 P0 → D24/D25 + 工程整理 → prerelease 发包 → 反馈期 feature phase  
...
| D25 | `+` 禁用可发现性 | **Pre-release 纳入**：灰显时 **Tooltip** 显示 `PassesPlacementGate` 的 `disabledReason`（非 Host Tab、无背包、不支持摆放等） |
| D26 | `+` 单次规划 | **P0 纳入**（P0-PR-02）：`PlaceRecipe` **禁止** preview + execute **各跑一遍**完整 `TryPlaceRecipe`；**一次规划、一次执行**，或 execute 路径复用 dry-run 的 `bestPlan` |
...
| US-C-PR-01 | 助手内分类搜索（**P0**） | 提交 query 后 **无 >200ms 主线程卡顿**；见 §9.PR.0 P0-PR-01 |
| US-C-PR-02 | 工作台点 `+`（**P0**） | 有料 **<100ms 体感**摆料或 toast；缺料 **<200ms**；见 §9.PR.0 P0-PR-02 |
...
**发包门槛**：**P0-PR-01 / P0-PR-02 未清偿前不得发 prerelease**（见 §9.PR.0）。

**原则**：

| 做 | 不做 |
|----|------|
| **P0** 搜索卡顿 + 工作台 `+` 延迟（§9.PR.0） | Phase 2c、Phase 3 新能力 |
| Phase 2a.3 **Pre-release 子集**（D24、D25） | 新机器 Host、平衡性、配方大改 |
| 接入清单文档 + REX/IE2 边界注释 | 无关大 refactor |
| modinfo **prerelease** 版本号 + 冒烟 | Hide 不 Destroy（反馈期） |

#### 9.PR.0 P0 阻塞项（发 prerelease 前必解）

| ID | 现象 | 影响 | 状态 |
|----|------|------|------|
| **P0-PR-01** | 合成助手内 **搜索提交后主线程卡顿**（数百毫秒～数秒，体感「游戏卡住」） | US-C02、§10 查询指标；玩家无法流畅查配方 | **未解决** |
| **P0-PR-02** | **工作台** Host 点配方卡 **`+` 后明显延迟** 才摆料 / 提示缺料 | US-C07、US-C08；核心 JEI 闭环体验 | **部分缓解**（2b 仅修缺料路径 1～2s，成功路径与双次执行仍在） |

##### P0-PR-01 — 合成助手搜索卡顿

| 项 | 说明 |
|----|------|
| **复现** | 打开合成助手 → 当前 Category 条目较多时 → 点 **搜** 或从历史/筛选带入 query → `PopulateBlocksList` 刷新列表时帧率骤降或短暂无响应 |
| **相关代码** | `RecipaediaCraftingOverlayDialog.PopulateBlocksList` → `RecipaediaSearchEngine.Filter(items, categoryId, query)`；对每个候选 `RecipaediaSearchIndex.GetDocument` + 全表达式匹配 + 排序 |
| **与全屏差异** | 全屏 `RecipaediaEXScreen` 同引擎，但 Overlay **打开即 `PopulateBlocksList`**、分类切换亦全量重建 `ListPanelWidget`；大分类下成本叠加 |
| **Phase 2a.2 备注** | 曾记「搜索性能优化（独立任务）」→ **现升为 P0**，发版前必做 |
| **修复方向（可组合）** | ① 确认 **仅搜按钮/Enter 触发 Filter**（非每帧 `Update` 误触发）；② Category 级 **文档/分数缓存** 或复用已预热索引；③ 短 query **DisplayName 前缀剪枝** 再进 Parser；④ 结果 **分帧** `AddItem`（若 List 支持）；⑤ 打开助手时 **先空列表、下一帧再填** |
| **验收** | US-C-PR-01：工业分类 + query `铁` / `copper`：**主线程无 >200ms 可感知卡顿**（与 §10 对齐）；连续搜 5 次无累积劣化 |

##### P0-PR-02 — 工作台 `+` 点击延迟

| 项 | 说明 |
|----|------|
| **复现** | 工作台打开助手 → Host Tab 有形配方 → 背包有料或缺料 → 点 **`+`** → **可感知等待** 后才有摆料 / toast |
| **相关代码** | `RecipaediaCraftingOverlayDialog.PlaceRecipe`：**连续两次** `TryPlaceRecipe(..., execute: false)` 与 `(..., true)`；REX `CraftingGridPlacementPlanner.TryPlace` 遍历 `recipe.TransformedIngredients` × `TryBuildPlan`；IE `CraftingOverlayIngredientBridge` / `RecipesUtils.Expand*` 在规划路径仍可能被调用 |
| **2b 已做** | 原版原料 **缺料-only** 路径：`RecipesUtils` craftingId 索引、Expand/缺料文案缓存、布局 **早退**（见 Phase 2b 验收期修复表）— **不覆盖** 成功摆放、部分填充、工业原料、**双次 TryPlace** |
| **修复方向（D26）** | ① **`PlacementPlan` 复用**：dry-run 产出 `bestPlan`，`execute=true` 只执行转移 + `UpdateCraftingResult`，**不二次枚举 layout**；② `EnsureTransformedLayouts` / `TryBuildPlan` **按 recipe 实例缓存**；③ 点击 `+` 至反馈前 **防抖**（禁连点）；④ 与 P0-PR-01 无关的 IE 展开缓存按需补 |
| **验收** | US-C-PR-02：工作台 9 格常见配方（铁块、铁砧）、背包有料：**点 `+` 至物品进格或 toast < 100ms 体感**；缺料路径 **< 200ms**（§10 扩展） |

#### 9.PR.1 时间路线（修订）

```text
15:00  定范围；P0-PR-01 / P0-PR-02 排期优先
15:15  ├─ P0 性能：搜索卡顿 + 工作台 + 延迟（D26 单次规划）（≈2～3h，发版门槛）
17:30  ├─ A 体验快赢：D24 搜索词 + D25 Tooltip（P0 通过后）
18:30  ├─ B 工程整理：接入清单 + 边界注释（≈45min）
19:00  └─ C Rebuild Release + 冒烟（含 US-C-PR-01/02）
19:30  发包说明、CHANGELOG、tag；双 mod 成对发布
20:00+ 缓冲：仅 P0 崩溃 / 性能回退
```

**铁律**：**P0-PR-01/02 未过验收不发包**；发包后除 P0 回退外不叠新 feature。

#### 9.PR.2 体验打磨（Track A）

| ID | 项 | 决议 | 风险 |
|----|-----|------|------|
| A1 | **记住搜索词**（D24） | ✅ 发前做 | 低；prerelease 少「助手不好用」噪音 |
| A2 | **`+` 灰显 Tooltip**（D25） | ✅ 发前做 | 低；`PassesPlacementGate` 已有 `disabledReason` |
| A3 | **Hide 不 Destroy** | ❌ 发后（2a.3 余项） | 中；易引入焦点 / 残留 Dialog |
| — | Shift 多组 `+`、R/U、Ghost 拖拽 | ❌ Phase 3 | — |
| — | 合并双次 `TryPlace`（D26 / P0-PR-02） | ✅ **P0 必做** | 见 §9.PR.0；行为不变、仅去重复计算 |

#### 9.PR.3 工程整理（Track B，零 / 极低运行时风险）

**目标**：不动大行为，让下次加机器、开 Phase 有 **固定模板**。

| ID | 项 | 说明 |
|----|-----|------|
| B1 | **工业机器 Placement 接入清单** | 主仓 `docs/guides/合成助手-工业机器接入清单.md`（半页）：Widget → `GetPlacementTarget` → Adapter → Target → 冒烟 3 条 |
| B2 | **会话态收口** | 与 A1 同做：`RecipaediaCraftingOverlaySessionState` 为 **唯一** 会话态入口（分类、滚动、搜索） |
| B3 | **REX / IE2 边界注释** | `IndustrialModLoader`、`PlacableRecipeAdapter` 各一行：REX = Overlay + 协议 + 有形默认算法；IE2 = Target + Planner + 桥接 |
| B4 | **版本标识** | `modinfo.json` 可识别 prerelease（如 `preview6` / 日期 tag）；依赖 `com.recipaediaex` 与 REX 包 **版本成对** |
| — | `FormatMissing` / `FormatMissingIngredient` 合并 | **发后** tech debt；发前仅注释，不 refactor |
| — | `FluidRecipaediaDescriptionScreen` 迁通用基类 | **发后**；非阻塞 |
...
| # | 检查项 |
|---|--------|
| 1 | **Rebuild Release** → `SCIE.scmod` + **RecipaediaEX** 成对进 Mods |
| 2 | 工作台 `+`：有料 / 缺料各 1 次 |
| 3 | 压板机（One2One）`+` 1 次 |
| 4 | 流体图鉴：**Esc → Esc** 回主菜单（TD-01 / v0.14） |
| 5 | toggle 助手两次：若已做 A1，确认 **搜索词仍在** |
| 6 | 非 Host Tab：`+` 灰显且 Tooltip 可读（若已做 A2） |
| 7 | **P0-PR-01**：分类内搜索 `铁` 无卡顿 |
| 8 | **P0-PR-02**：工作台 `+` 有料/缺料响应 < §10 指标 |

#### 9.PR.5 发后 feature phase 入口（非本晚范围）

反馈期以 **P0/P1 hotfix** 为主；体验与工程可并行：

| 窗口 | 建议 |
|------|------|
| **W1** | hotfix + **Hide 不 Destroy**（2a.3 余项） |
| **W2** | **Shift 多组 `+`** 或 **RecipeSlotWidget 联动**（二选一开 feature） |
| **W3** | **Phase 2c 熔炉** 或未接 Host 机器（按玩家反馈热度） |

#### 9.PR.6 实现清单（Pre-release）

- [ ] **P0-PR-01** 合成助手搜索卡顿（§9.PR.0）
- [ ] **P0-PR-02** 工作台 `+` 延迟 + **D26** 单次规划（§9.PR.0）
- [ ] Phase 2a.3 Pre-release 子集（§2a.3.1）
- [ ] `docs/guides/合成助手-工业机器接入清单.md`
- [ ] REX/IE2 边界注释（B3）
- [ ] modinfo / CHANGELOG prerelease 标记（B4）
- [ ] Track C 冒烟通过
- [ ] 群公告：Beta 范围、已知限制（2c/Shift/R/U 未含）、反馈渠道
...
|------|--------|------------------|------|
| **记住搜索词**（D24） | P1 | ✅ **纳入** | `SessionState.SearchQuery`；`CaptureSessionState` / 构造恢复 |
| **`+` 禁用 Tooltip**（D25） | P1 | ✅ **纳入** | `RecipaediaOverlayDescriptorActionBar.RefreshGateState` 绑定 `disabledReason` |
| **Hide 不 Destroy** | P2 | ❌ **暂缓** | 改 `RecipaediaCraftingOverlayController` 生命周期；反馈稳定后再做 |
| **单次规划 D26**（P0-PR-02） | **P0** | ✅ **纳入** | `PlaceRecipe` 复用 dry-run plan；见 §9.PR.0 |

#### 2a.3.1 实现清单

- [ ] **D26**：`CraftingGridPlacementPlanner` / `PlaceRecipe` 单次规划（P0-PR-02，**先于** D24/D25）
- [ ] `RecipaediaCraftingOverlaySessionState`：增加 `SearchQuery`（与 D23 同文件收口）
- [ ] `CaptureSessionState` / Dialog 构造：写入并恢复搜索词；**换分类 / 新搜索提交** 行为与全屏一致（换分类清空）
- [ ] `RecipaediaOverlayDescriptorActionBar`：禁用 `+` 时显示门控原因 Tooltip
- [ ] （反馈期）`RecipaediaCraftingOverlayController`：toggle **Hide** 而非 `Remove` + 重建
- [ ] （P0-PR-01）Overlay 搜索 / `PopulateBlocksList` 性能（见 §9.PR.0）
...
| US-C2a.3-06 | 工作台 `+` 无延迟（P0-PR-02） | 同 US-C-PR-02 |
...
| 查询（全屏） | < 100ms / 5000 候选 |
| 查询（**合成助手 Overlay**） | **P0-PR-01**：分类内搜索提交后 **< 200ms** 主线程（US-C-PR-01）；大分类与全屏同量级 |
| **`+` 响应（工作台）** | **P0-PR-02**：点 `+` 至摆料或缺料 toast **体感 < 100ms（有料）/ < 200ms（缺料）**（US-C-PR-02） |
...
| Overlay 搜索卡顿 | **P0-PR-01**：Filter 全量扫描 + List 重建；见 §9.PR.0 修复方向 |
| 工作台 `+` 双次 TryPlace | **P0-PR-02 / D26**：`PlaceRecipe` preview+execute 重复规划；见 §9.PR.0 |
