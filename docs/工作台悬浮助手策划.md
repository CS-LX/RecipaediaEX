...
> **版本**：v0.6  
> **状态**：**Phase 1 已收尾**（2026-06-16）；**Phase 1.5 已收尾**（2026-06-16，见 §9.1）；Phase 2a+ 待实现  
...
| **合成助手** | `RecipaediaCraftingOverlayDialog` | 同屏搜配方、看布局、Descriptor 链式导航；**+** 待 Phase 2a | ✅ |
...
2. **上下文感知**（基元 V4）：条目搜索 **全局（All Blocks）**；选中条目后，二级预览按 **Crafter Tab 分页**（默认 Tab = 当前 Host 方块类型），**不按 `GridWidth` 裁剪**（非搜索 Token）。
...
- **+ 按钮**（Phase 2a）：配方实现 `IPlacableRecipe` 且 Host 提供 `IRecipePlacementTarget` 时启用；否则禁用并 Tooltip（基元 E2）。
...
│ │  ┌──────────────┐  ┌──────────────┐              │  │
│ │  │ DescriptorSlot│  │ DescriptorSlot│  …          │  │  ← RenderTransform 缩放，不改内部布局
│ │  └──────────────┘  └──────────────┘              │  │
│ └──────────────────────────────────────────────────┘  │
│                              [ + 填充 ]（Phase 2a）    │
└────────────────────────────────────────────────────────┘
