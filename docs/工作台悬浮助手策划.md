...
> 文档分层后，本文件归属 [内部开发文档](internal/README.md)，作为历史策划、设计决策和验收记录保留。玩家向说明见 [合成助手](user/crafting-overlay.md)，外部接入说明见 [合成助手接入](api/crafting-overlay-integration.md)。

> **版本**：v0.32  
> **状态**：**`2.0.0.0` 正式版已发**（2026-07-12）；含 preview8 + **W2 / W3** + InterceptBus  
> **下一里程碑**：**W6 搁置**（见下）；2.0.x 后续 polish  
...
| 项 | 说明 |
|----|------|
| **状态** | **搁置** — 在探索出与 JEI 对齐且体验可接受的可行方案前 **不实施** |
| **预计重启** | **`2.0.0.0` 正式版发布之后**（非 preview 线） |
| **背景** | 2026-07 曾按 D29/D30 试做「打开即推荐 + 按材料反查」；黑盒体感 **未改善、反似倒车**（有背包料即替换全库浏览、整包 AND 反查、粗 OR 推荐噪声大），与 [`合成助手-JEI对标与基元语句.md`](合成助手-JEI对标与基元语句.md)「右侧 Ingredient 列表常驻」冲突 |
| **代码** | 试做已 **回退**；现网行为 = preview8（全库搜 + All Blocks 默认浏览） |
| **需求仍有效** | 酸菜小咸鱼 F7～F9 痛点保留；重启前须重审产品形态（可选 Tab / 单料 U 式反查，**非**默认劫持主列表） |

### 接续开发入口（下一 Agent）

| 项 | 说明 |
|----|------|
| **已发 REX** | **`2.0.0.0` 正式版**（2026-07-12）；含 preview8 + W2/W3 + InterceptBus；详见 [`CHANGELOG.md`](CHANGELOG.md) |
| **依赖模组侧** | 各依赖模组自行对齐 `modinfo` 中 `com.recipaediaex`、子模块/包发布；**Host 接入进度不在 REX 仓维护** |
| **代码锚点** | Overlay：`RecipaediaCraftingOverlayDialog` / `Controller`；Placement：`IRecipePlacementTarget` / `PlacableRecipeAdapter.IsPlacable` / `FormattedGridPlacementPlanner`；搜索：`RecipaediaSearchEngine` |
| **下一优先（REX）** | **W6/4c 仍搁置**（2.0.x 方案评审）；依赖模组 Host 扩展由各模组自行规划 |
> **前置**：[`图鉴搜索功能策划.md`](图鉴搜索功能策划.md) Phase 1/2 已交付（搜索引擎、索引、筛选 Dialog 可用）  
> **对标参考**：[`合成助手-JEI对标与基元语句.md`](合成助手-JEI对标与基元语句.md)（Minecraft JEI 解构、基元语句、依赖模组扩展依据）

---

## 1. 玩家反馈摘要

### 1.1 社群玩家 A（2026-06 社群讨论 · preview 前）
...
**产品定位**：本模组（依赖 REX 的内容模组）对标 **Minecraft JEI** 的 **悬浮窗 + 自动摆放（+）+ 材料驱动浏览** 体验；REX 提供协议与 UI 壳，专有机器逻辑由内容模组接入。
...
来源：QQ 群玩家 **洛贼**（模组方块 / 合成助手体验）；开发者 LX.. 已 OK。

| # | 原话要点 | 诉求归类 | 与现网差距 |
|---|----------|----------|------------|
| F12 | 输入完改一下放大镜，**自动搜索**，省得再按一下 | **输入即搜**（少一次提交） | 仅点击 `Search` 才 `ApplySearchFromInput()` |
| F13 | 车间机床等打开 JEI **默认应是全部不是地形** | **合成 Host 下默认 All Blocks** | `DefaultOverlayCategoryId` 取 `categoryIds[1]`，部分模组下常为 **Terrain/地形** |
...
| 自动摆放 | Phase 2a/2b + W2/W3 已交付 | ✅ 有形合成、熔炼输入区、One2One / 反应釜 / 多槽 Host 等（由依赖模组实现） |
...
| 内容模组专有机器 | Phase 2b | ✅ 预览 + **`+`**（依赖模组实现 Target） |
...
| D9 | Phase 2a 绑定 | `CraftingTablePlacementTarget` 绑 **`ComponentEXCraftingTable`**（内容模组可继承扩展） |
...
| D12 | 内容模组 Descriptor | **一并改** Navigator 构造签名（REX + 内容模组全部 `[RecipeDescriptor]`） |
| D13 | `GridWidth` 预览过滤 | **废止**（Phase 1.5）：有形合成（工作台 / 扩展合成台）为 **同一 Recipe Category**，不按 3/4/5 拆预览 |
...
内容模组可另有 **合成提示**（如 `CrafterHints` 开关），与合成助手 **独立**（助手开/关为会话态，见 §6.8）。
...
5. **可扩展自动摆放**（基元 E1/E2）：REX 定义 **Placement 协议**；有形合成、One2One、反应釜等由内容模组注册 `IRecipePlacementTarget`；**不在 REX 核心写内容模组专有逻辑**。
...
2. 按 `X` → REX Hook 经 **`RecipaediaEventBus`** 发布「打开全屏图鉴」类事件；内容模组（如科技树 Screen）**订阅** 后 `SwitchScreen("Recipaedia")` 或等价逻辑，**不在 REX 外重复监听键位**。
...
```text
YourMod/Modules/RecipaediaEX/Overlay/
  ReactorPlacementTarget.cs          # ReactionEquationRecipe
  One2OnePlacementTarget.cs          # 单槽 Crafter 等
...
|---------------|------|-----------------|
| 工作台 / 扩展合成台 | ✅ | `CraftingTablePlacementTarget`（`ComponentEXCraftingTable`） |
| 原版 / 扩展熔炉（`ComponentEXFurnace`） | W3 | `FurnacePlacementTarget` + `RecipaediaFurnaceWidget`（REX） |
| 扩展熔炉 Widget | W3 | 同上 `FurnacePlacementTarget`（内容模组 Widget 接 Host） |
| 方程类反应容器 | ✅ | `IRecipePlacementTarget`（内容模组） |
| 纯箱子 Modal | ❌ | — → 快捷键走全屏图鉴 |
...
**设计约束（v0.3）**：REX 核心 **不得** 出现 `FluidVolume`、`FluidTank`、`ITank` 等流体/内容模组专有字段或枚举值——与 [`recipaedia-rex-anti-patterns.mdc`](../../../.cursor/rules/recipaedia/recipaedia-rex-anti-patterns.mdc) §1 一致。流体、罐体、化工方程的 **Quantity 语义与槽位物理含义** 由 **内容模组** 的 `IPlacableRecipe` 适配器 + `IRecipePlacementTarget` 实现解释；REX 只提供 **与领域无关的寻址 + 数量** 载体。
...
| 熔炼输入区 | `GridCell`（第一行） | 0–35，**仅映射** `0..m_furnaceSize-1` | REX `FurnacePlacementTarget` → **个数**；**非** One2One |
...
| One2One 单槽 | `ContainerSlot` | 0 | 内容模组 Target → **个数**（单槽 Crafter 等，**≠ 熔炉**） |

内容模组在 **各自仓库** 维护「AddressIndex → 物品槽 / 罐体 / …」映射表；REX 不定义该表。
...
| `OriginalSmeltingRecipe` | REX 内置适配（W3） | `GridCell` 第一行 + **Transform 平移**（对齐 `MatchRecipe` / `FormattedRecipe.Match`） |
| `ReactionEquationRecipe` | 内容模组 | `ContainerSlot` 列表；Target 解释罐体 |
| `One2OneRecipe` | 内容模组 | 单 `ContainerSlot`（**勿与熔炉混用**） |
...
**内容模组 `ReactorPlacementTarget`**（绑定机器 Component）：
...
**禁止**：在 REX 核心写反应釜/灌装机等内容模组专有逻辑，或在 REX 类型上出现流体专有成员 → 内容模组实现 Target 并在 Widget 的 `GetPlacementTarget()` 返回。
...
REX Hook（非 Overlay 场景按 X）
  ...
  → 内容模组订阅者（如依赖模组 Subsystem）执行 SwitchScreen / 自定义逻辑
```

REX **不** 硬编码 `SwitchScreen("Recipaedia")` 到具体 Screen 名；默认订阅者可由 REX 或依赖模组注册，**YAGNI 一种事件类型即可**。
...
| `CrafterHints`（合成提示） | 依赖模组用户设置，与合成助手 **无关** |
...
| Phase 2b | `One2OneRecipe` + 单槽 Crafter 等 | 单输入槽（One2One，与熔炉无关） |
| Phase 2c | `OriginalSmeltingRecipe` + `ComponentEXFurnace`（内容模组可继承扩展） | **熔炼输入区** `+`：多格、多料、**Transform 平移**；仅写输入格（不写燃料/产物/残渣） |
...
| US-C06 | 非合成场景按 X | 经 **EventBus** 打开全屏图鉴（依赖模组订阅者行为与现网一致） |
...
- [x] `IRecipaediaRecipeNavigator` + `RecipeDescriptorRegistry`；**全库 Descriptor 构造迁移**（REX + 依赖模组）
...
- [x] Host 接入：扩展合成台 + 方程类预览；内容模组 `*HostHelper`
- [x] i18n：`ContentWidgets.RecipaediaCraftingOverlay.*`

**验收记录**：2026-06-16 手工冒烟通过（扩展合成台 / 反应釜；条目点击 → 左侧配方弹窗；搜索与筛选与全屏一致）。
...
| `RecipaediaCraftingTableContextHelper.cs` | 逻辑 **内联** 至 `RecipaediaCraftingTableWidget.GetCraftingContext()`（内容模组侧 HostHelper 同理去掉 `gridWidth` 参数） |
...
| Tab 排序 | **当前 Host Crafter 置顶** → 有形合成类（工作台 / 扩展合成台 **分 Tab，不合并品牌**，Q1.5-1）→ 熔炼类 → 内容模组专用 Crafter |
...
// 2. 分组：key = ICrafter.GetCraftingId（Crafter 设备同 BlockIndex 不合并）
...
| ID | 故事 | 验收 |
|----|------|------|
| US-C1.5-01 | 废止 GridWidth 后点「铁块」 | 工作台 / 扩展合成台 Host 下，**有形铁块配方在对应有形 Tab 均可见**；**无**「仅因格宽被隐藏」 |
...
- [x] 内容模组 HostHelper / 各 Host：去掉 `gridWidth` 实参  
...
- **不**将有形合成类合并为单个「超级 Tab」（工作台 / 扩展合成台 **分 Tab**；熔炉 / 反应釜等仍各为独立 Tab；仅 **废止按格宽拆预览**）  
...
- [x] Host：`RecipaediaCraftingTableWidget` / 扩展合成台 `GetPlacementTarget()`
...
**优先级**：P1（紧接 Phase 2a；建议在 Phase 2b 专有机器摆放前完成，避免反应釜等沿用「全局 + / 取第一条」的过渡语义）
...
- **不**在 REX 核心新增内容模组专有摆放逻辑（仍走既有 `IRecipePlacementTarget`）
...
### Phase 2b — 内容模组专有机器自动摆放（P1，已收尾）

**动机**：反应釜 / One2One 机器 Host 已能预览配方，但 `GetPlacementTarget()` 未接，`+` 灰显。Phase 2b 在 **依赖模组** 落地 Target + 适配器，REX 仅扩展 `PlacableRecipeAdapter.Register`。
...
- [x] `ReactorWidget` + 单槽 One2One Host 接线
- [x] `CraftingOverlayIngredientBridge`（模组自定义合成 ID 注入；REX 侧桥接）

| ID | 验收 |
|----|------|
| US-C11～C13 | 反应釜 `+` 物品/流体/FindEquation |
| US-C2b-01 | 单槽 One2One Host `+` 填单槽 |
| US-C2b-02 | 非 Host Tab `+` 禁用 |

**验收记录**（2026-06-16）：生存模式冒烟全通过——单槽 One2One Host 填槽/替换/缺料提示；工作台有形合成 `+`（含原版与模组自定义原料）；非 Host Tab `+` 灰显；`+` 成功后详情自动收起；缺料文案正确；原版原料缺料提示无 1～2s 卡顿。
...
| One2One 模组配方误填铁矿粉 | 原料匹配改用 `RecipesUtils.CompareIngredients`，不再在展开候选中取「数量最多堆叠」 |
...
| 模组自定义合成 ID 配方 匹配失败 / 点击 `+` 崩溃 | `CraftingOverlayIngredientBridge` + 模组 `ModLoader` 注入；自定义原料解码 改 `throwIfNotFound: false` |
...
| 序 | ID | 项 | 说明 | 估时 |
|----|-----|-----|------|------|
| 1 | **B1** | **Placement 接入清单模板** | 各依赖模组自维护：Widget → Target → Adapter → 冒烟（**不在 REX 仓**） | — | ✅ |
| 2 | **B3** | **REX / 依赖模组边界** | `ModLoader`、`PlacableRecipeAdapter`（REX）、模组 Placable 适配器 | — | ✅ |
| 3 | **B2** | **会话态收口** | 与 **A1（D24）** 同 PR：`RecipaediaCraftingOverlaySessionState` 统一分类 + 滚动 + 搜索词 | 30min（含 A1） | ✅ |
| 4 | **B5** | **子模块指针同步** | 使用子模块的宿主仓库对齐 REX commit | — |
| 5 | **B4** | **版本标识 + 发包物** | REX `modinfo` / CHANGELOG / tag / Release 产物 | — |
...
2. 若需新配方类型 → PlacableRecipeAdapter.Register（模组侧）
...
| # | 检查项 | 结果 |
|---|--------|------|
| 1 | **Rebuild Release** → REX + 依赖模组包 成对进 Mods | ✅ 2026-06-19 |
| 2 | 工作台 `+`：有料 / 缺料各 1 次 | ✅ |
| 3 | 单槽 One2One Host`+` 1 次 | ✅ |
...
**加测（D 项，2026-06-19）**：扩展合成台有形 `+`、反应釜 `+`（物品/流体缺料提示）、非合成场景 `X` 进全屏图鉴 — **均通过**。
...
| **W6** | **Phase 4c** 打开即推荐 + 原料反查入口 | F7～F9（D29、D30） | 2～3d — **⏸ 搁置至 `2.0.0.0` 正式版后** |
| **W2** | **长按 `+` 连续放置**（JEI 式多组 pattern / 单槽灌满） | — | ✅ **已做**（2026-07-11） |
| **W3** | **Phase 2c 熔炼输入区**（`FormattedGridPlacementPlanner` + 熔炉 Host） | 按热度 | ✅ **已做**（2026-07-11） |

**推荐实施顺序**：W1 → W4 → **W4.5** → W5 → ~~W6~~ → **W2** ✅ → **W3**（W6 搁置，见文首说明）。

**发包建议**：W4.5 完成后可发 **REX `preview7`**（CHANGELOG：默认 All + 输入即搜）；W5/W6 可合并为 **preview8** 或继续 patch 迭代。（W6 未纳入 preview8 之后发版。）
...
- [x] **B4** modinfo / CHANGELOG / tag；REX `preview6` 已发（2026-06-19）；依赖模组发包由各宿主维护者自理
...
- [x] （反馈期）`RecipaediaCraftingOverlayController`：Hide 不 Destroy（preview7）
...
### Phase 2c — 熔炼输入区（P1，W3）

**动机**：preview8 已交付工作台/扩展合成台/方程类 Host 的 `+`，但 **熔炉面板无 Host**、`OriginalSmeltingRecipe` 无 `IPlacableRecipe`，玩家开炉仍无法同屏助手 + 摆料。社区与黑盒共识：**熔炉不是「单槽 One2One」**——扩展熔炉常见 **2～4 个输入格**，多料配方为 **多料 + 格位**（如 `"aab"`、`"abb"`），且匹配管线 **支持 Transform 平移**（玩家可手动在输入格间平移图案）。

**产品原则（v0.29 修订，废止「单槽 +」表述）**：

| 原则 | 说明 |
|------|------|
| **≠ One2One** | 熔炉走 `OriginalSmeltingRecipe` + 输入区格位，**不**复用 `One2OnePlacementTarget` |
| **多输入格** | 仅写入 `0 .. m_furnaceSize-1`；**禁止**触及燃料 / 产物 / 残渣格 |
| **Transform** | 与工作台相同，遍历 `TransformedIngredients` 选最优可摆放变体（对齐 `CraftingRecipesManager.MatchRecipe` / `FormattedRecipe.Match`） |
| **平移语义** | 图案可在输入格内 **左右平移**；**无**工作台式 N×N 大方形格，但 **有** 沿第一行（`gridIndex/6==0`）的 shift |
| **Host 门控** | 仅当前熔炉 Crafter Tab 且 `GetPlacementTarget()` 可用时 `+` 启用（D18） |
| **统一 Planner** | `CraftingGridPlacementPlanner` 升格为 **`FormattedGridPlacementPlanner`**；工作台与熔炉共用取料/缺料/执行，**格位映射策略**分 `SquareCrafting` / `FurnaceInputRow` |

**架构（REX / 依赖模组）**：

```text
FormattedRecipe（OriginalCraftingRecipe | OriginalSmeltingRecipe）
  → FormattedGridPlacableRecipe（统一 IPlacableRecipe）
  → FormattedGridPlacementPlanner.TryPlace(context, recipe, …)
       ├─ SquareCraftingMapper   → CraftingTablePlacementTarget
       └─ FurnaceInputRowMapper  → FurnacePlacementTarget
```

**实现清单**：

- [x] REX：`FormattedGridPlacementPlanner`（工作台 **零行为回归**）
- [x] REX：`PlacableRecipeAdapter.IsPlacable` 注册 `OriginalSmeltingRecipe`
- [x] REX：`FurnacePlacementTarget`（`ComponentFurnace` / `ComponentEXFurnace`）
- [x] REX：`RecipaediaFurnaceWidget` + `RecipaediaEXLoader.OnModalPanelWidgetSet` 替换原版 `FurnaceWidget`
- [x] 依赖模组：扩展熔炉 Widget 接 `IRecipaediaOverlayHost` + `FurnacePlacementTarget`
- [x] 各模组接入清单增补熔炉行

**明确不在 W3**：

- 高炉 / 真空炉 / 合金炉等非 `OriginalSmeltingRecipe` 设备
- 燃料格自动填充
- W6 材料推荐
...
| ID | 故事 | 验收 |
|----|------|------|
| US-C2c-01 | 原版或铁炉 Host，熔炉 Tab，`+` | 单料 `"a"` 进输入格；`UpdateSmeltingRecipe` 刷新 |
| US-C2c-02 | 炼钢 `"aab"` @ 3 格铁炉 | 铁+铁+煤进 **输入格 0,1,2**；或平移变体进 1,2 等 **仍在输入区内** |
| US-C2c-03 | 工作台 Host 看熔炉 Tab | **只读**；`+` 灰显（D18 回归） |
| US-C2c-04 | 长按 `+`（W2） | 输入格广度优先灌满；**不**写燃料格 |
| US-C2c-05 | 关熔炉 Modal | Overlay Dismiss（D28 回归） |

**验收记录**（2026-07-12）：生存模式手工黑盒 **US-C2c-01～05 全通过**；原版熔炉 + 依赖模组扩展熔炉；Track C 工作台/One2One 回归无回归。
...
- [x] Host 关闭路径统一调用 `RecipaediaCraftingOverlayController.DismissForModalWidget(...)`（preview7）
- [x] `RecipaediaOverlayHostUi.EnsureToggleButton`：双重检查 `GetCraftingContext()`（preview7）
- [x] 验收：US-C4-04、US-C4-05；叠加 W1 Hide 不 Destroy 回归（2026-06-25 黑盒通过）
...
- [x] `RecipaediaCategoryCatalog`：`DefaultOverlayCategoryId` 返回 `"All Blocks"`（preview7）
- [x] `RecipaediaCraftingOverlayDialog`：debounce 250ms + Enter 立即 `ApplySearchFromInput()`（preview7）
- [x] 复用 P0-PR-01 Filter 缓存；连续键入 US-C-PR-01 **回归**（2026-06-25 黑盒通过）
- [ ] i18n：placeholder 可加「输入即搜」（可选，未做）
- [x] 验收：US-C4d-01～04（2026-06-25 黑盒通过）
...
- [x] `RecipaediaCategoryCatalog.GetOverlayGlobalSearchCandidates` + `RecipaediaCraftingOverlayDialog`：`PopulateBlocksList` 有 query → 全库 Filter（preview8）
- [x] Category 切换：有搜索词时 **不清空**、不刷新列表；浏览模式下换分类刷新列表（Q4b-1，preview8）
- [x] i18n：`RecipaediaCraftingOverlay:12` 全库搜索、`13` placeholder（preview8）
- [x] 验收：US-C4-01（2026-06-25 黑盒通过）

#### Phase 4c — 材料上下文与原料反查（P1～P2，W6）— ⏸ **搁置**

> **2026-07**：试做回退；**搁置至 `2.0.0.0` 正式版后**，待 JEI 对齐方案评审通过再开发。文首「W6 搁置说明」。
...
| # | 问题 | 倾向 |
|---|------|------|
| Q4b-1 | 全局搜模式下切换 Category 是否仍清空搜索？ | ✅ **不清空**；Category 仅影响浏览模式（preview8） |
...
- [x] **长按 `+` 连续放置**（W2）：`PlacementLongPressRepeater` 二次缓入 1s/40 组；工作台广度优先每格 +1；连放不清格；绿色 `+` 悬停提示（`RecipaediaCraftingOverlay:14`）
- [ ] Ghost 拖拽（过滤器、样板；基元 P10）
- [ ] Shift 多组、`MaxSets`（**已由 W2 长按连放替代**；不再计划 Shift+）
...
| 反模式 | 遵守 `recipaedia-rex-anti-patterns.mdc`：内容模组领域语义不进 REX 核心 API |
...
| 快捷键双监听 | **仅 REX Hook**；依赖模组改 **EventBus 订阅** |
...
| v0.2 | 2026-06-16 | 整合 JEI 对标、基元语句、接口修订（Placement dry-run/execute、`IPlacableRecipe`）、化工方程类场景与 Phase 2b、区分 `CrafterHints`；废止「化工 Phase 3 不做 +」 |
...
| v0.4 | 2026-06-16 | 评审决议：Navigator 轻量解耦、All Blocks、弃 `@grid`/ProcessType、EventBus+X Hook、JEI 式右栏+Grid 预览、AdHoc 排除、ComponentEXCraftingTable、流体罐低优先级、内容模组 Descriptor 迁移 |
...
| v0.10 | 2026-06-16 | Phase 2b **实现待验收**（内容模组 Target/Planner/Host 接线） |
...
| v0.19 | 2026-06-16 | **Track B** B1/B3/B2：**接入清单**（各模组 docs）、边界注释、**D24** 搜索词会话记忆 |
...
| v0.25 | 2026-06-25 | **preview7 交付**：W1 Hide 不 Destroy、W4 生命周期/门控、W4.5 默认 All + debounce 搜；CHANGELOG；黑盒验收 US-C4-04/05、US-C4d-01～04、US-C2a.3-04 |
| v0.26 | 2026-06-25 | **preview8 交付**：W5 Phase 4b 助手全库搜索；`GetOverlayGlobalSearchCandidates`；Q4b-1 定稿；黑盒 US-C4-01 |
| v0.27 | 2026-07-08 | **W6 搁置**：Phase 4c 试做回退；黑盒未达标且与 JEI「列表常驻」冲突；**预计 `2.0.0.0` 正式版后**再评审 F7～F9 可行方案；下一开发优先 W2/W3 |
| v0.28 | 2026-07-11 | **W2 交付**：长按 `+` 连续放置（`PlacementLongPressRepeater`）；有形合成格广度优先；`PlaceRecipe(clearGridBeforePlace, showFeedback)`；i18n `:14` 悬停提示；One2One 灌槽见依赖模组 Planner |
| v0.29 | 2026-07-11 | **Phase 2c / W3 语义修订**：废止「熔炉单槽 +」；明确熔炼 **多输入格、多料、Transform 平移**；W3 技术路线 `FormattedGridPlacementPlanner` + `FurnacePlacementTarget`；§6.5.3 / §7.1 / §9.PR.5 / US-C2c |
| v0.30 | 2026-07-11 | **W3 交付**：`FormattedGridPlacementPlanner`、`FurnacePlacementTarget`、`RecipaediaFurnaceWidget`；依赖模组扩展熔炉 Host；CHANGELOG Unreleased |
| v0.31 | 2026-07-12 | **W3 黑盒通过**：US-C2c-01～05；移除 `CraftingGridPlacementPlanner` 兼容层；`PlacableRecipeAdapter.IsPlacable` 替代 `TryAsPlacable` |
...
- 各依赖模组自维护 Host / Placement 接入清单（REX 提供协议与 §6 步骤）
